...
(define* (dbi-open txn name #:optional (flags 0))
...
(define* (cursor-get cursor #:optional (op 0) (key #f))
  (let ((key (unwrap-val (make-val key)))
        (data (unwrap-val (make-val))))
    (check-error
     ((foreign-fn "mdb_cursor_get" `(* * * ,int) int)
      cursor key data op))
    (wrap-val data)))
(define (cursor-first cursor key)
  (cursor-get cursor +first+ key))
